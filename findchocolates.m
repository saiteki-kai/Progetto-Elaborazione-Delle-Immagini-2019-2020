...
[centers, radii, metrics] = imfindcircles(s, [rmin rmax], ...
    'Method', 'TwoStage', ...
    'Sensitivity', 0.85, ... %0.9(mia)
    'EdgeThreshold', 0.1, ...%0.05(beppe)
    'ObjectPolarity', 'dark');
...
%REMOVEEXTERNALS remove circle with ten percent of black in neighborhood

keep = false(length(centers), 1);
for k = 1 : length(centers)
    circle = utils.cropcircle(mask, centers(k, 1), centers(k, 2), rmin, false);
    
    N = length(circle)^2; % area
    n = sum(circle == 0, 'all');
    if n/N <= 0.15
        keep(k) = 1;
    end  
